...
while(rs3.next()){%>
          ...
          <div class="to">INVOICE TO:</div>
            <h2 class="name">ID:<%=rs3.getString(1) %> </h2>
          ...
          <div class="email"><a href="mailto:john@example.com">EMAIL:<%=rs3.getString(2) %> </a></div>
              ...
              rs3.close();
               ...
               PreparedStatement ps4 = con.prepareStatement("SELECT * FROM customer INNER JOIN orders INNER JOIN payment ON orders.customer_ID = customer.customer_ID AND payment.payment_ID = orders.payment_ID WHERE customer.customer_ID = ? AND orders.order_ID = ?");
...
while(rs5.next()){%>
  ...
  <% 
  PreparedStatement ps7 = con.prepareStatement("SELECT SUM(total_price_order) FROM product INNER JOIN order_item INNER JOIN orders ON product.product_ID = order_item.product_ID AND order_item.order_ID = orders.order_ID WHERE order_item.order_ID = ?");
...
%>
	...
	<br><br>
		<a href="index.jsp">   <input type="button" value="okey" class="primary-btn order-submit" >   </a> 

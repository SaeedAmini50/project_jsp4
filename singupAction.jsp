...
if (rs1.next()) {
 ...
 ps = con.prepareStatement("INSERT INTO customer (customer_name, customer_last_name, email, Security_Question, customer_answer, password, customer_date) VALUES (?, ?, ?, ?, ?, ?, NOW())");
...
ps.executeUpdate();
 ...
 pstmt = con.prepareStatement("SELECT * FROM customer WHERE email=? AND password=? AND customer_name=? AND customer_last_name=? AND Security_Question=? AND customer_answer=?");

...
try{
	...
	if (password.equals(password2)){
	 ...
	 ps = con.prepareStatement("INSERT INTO admin (admin_name,admin_last_name,email,admin_phone,Security_Question,admin_answer,password,admin_date) values(?,?,?,?,?,?,?,now());");
...
ps.close();
	...
	String query = "SELECT * FROM admin WHERE email=? AND password=? AND admin_name=? AND admin_last_name=? AND admin_phone=? AND Security_Question=? AND admin_answer=?";
